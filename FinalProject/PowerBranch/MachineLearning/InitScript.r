...
NextDataset <- function(dataset, steps="month")
{
  ...
  for(pointName in newPoints)
  {
    ...
    neighbours1value <- dataset[which(dataset$date==nextDate & dataset$point==neighbours[1]),]
    ...
    neighbours2value <- dataset[which(dataset$date==nextDate & dataset$point==neighbours[2]),]
    ...
    neighbours3value <- dataset[which(dataset$date==nextDate & dataset$point==neighbours[3]),]
...
mainForMercury <- function(dataset = data.frame())
{
  ...
  write.csv(x = dataset, file = "./preparedDataset.csv", row.names = FALSE)
  
  #dataset <- read.csv(file = "./dataset_for_filling.csv")
  #PreviousSet <- read.csv(file = "./next_set.csv")
  #PreviousSet$date <- as.Date(PreviousSet$date, format = "%m/%d/%Y")
  #dataset$date <- as.Date(dataset$date)
...
fullset <- read.csv("./dataset.csv", fileEncoding = "UTF-8")
fullset$date <- as.Date(fullset$date, format = "%Y-%m-%d")

mainForMercury(fullset)